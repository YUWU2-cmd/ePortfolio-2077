      ...
      <div class="all">
        ...
        <div class="forget_box">
            ...
            <div class="description">
                Fill out your email address and verify code from sended email. You'll go to the instructions of reset your password. 
            </div>
            ...
            <!-- rules和prop属性都是校验相关的。
             ...
             <el-form ref="forgetFormRef" :model="forgetForm" :rules="forgetFormRules" label-width="0px" class="forget_form">
                ...
                <el-input class="input" v-model="forgetForm.verifyCode" placeholder="Fill in your verifyCode"></el-input>
                </el-form-item>
                <!-- 按钮区域 -->
                <div class="btn">
...
export default {
  ...
  methods: {
    ...
    verify () {
      console.log(this.$qs.stringify(this.forgetForm))
      this.$refs.forgetFormRef.validate(async valid => {
        if (!valid) return
        var data1 = this.$qs.stringify(this.forgetForm)
        console.log(data1)
        const { status: res } = await this.$http.post('/user/verify', data1, {headers:{'Content-Type':'application/x-www-form-urlencoded'}})
